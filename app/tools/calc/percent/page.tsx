...
export default function PercentCalculator() {
  ...
  useEffect(() => {
    if (values.totalValue1 && values.percentage1) {
      ...
      if (!isNaN(total) && !isNaN(percent)) {
        setValues((prev) => ({ ...prev, result1: (total * percent) / 100 }))
      }
    }

    if (values.totalValue2 && values.partialValue2) {
      ...
      if (!isNaN(total) && !isNaN(partial) && total !== 0) {
        setValues((prev) => ({ ...prev, result2: (partial / total) * 100 }))
      }
    }

    if (values.baseValue3 && values.changePercentage3) {
      ...
      if (!isNaN(base) && !isNaN(change)) {
        setValues((prev) => ({ ...prev, result3: base * (1 + change / 100) }))
      }
    }

    if (values.baseValue4 && values.newValue4) {
      ...
      if (!isNaN(base) && !isNaN(newVal) && base !== 0) {
        setValues((prev) => ({ ...prev, result4: ((newVal - base) / base) * 100 }))
      }
    }
  }, [
    values.totalValue1,
    values.percentage1,
    values.totalValue2,
    values.partialValue2,
    values.baseValue3,
    values.changePercentage3,
    values.baseValue4,
    values.newValue4,
  ])
